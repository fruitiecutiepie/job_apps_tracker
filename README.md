...
Its own search sits where the filters do on other views, and searches the notes themselves — every prep note and every captured line, whether or not it is open. Pick a result and it opens in the pane you are reading. That is a different question from the search on the board, which finds applications, and from `Ctrl`/`Cmd+F`, which searches only what is already open.
