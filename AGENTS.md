...
- The editor resolves an action through the same record, but as a draft: **Mark next action done** clears the draft next action and adds a row, and nothing is written until the dialog is saved. Every other control in that form behaves this way, and an immediate write would commit while other edits sit unsaved beside it. The section lists what is recorded, newest first, apart from the Notes textarea, and each row can be removed.
